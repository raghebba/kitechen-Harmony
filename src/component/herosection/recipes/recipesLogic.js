const parseRecipeString = (text) => {
  ...
  const linesBeforeIngredients = text.substring(0, titleEnd).split('\n').filter(line => line.trim() !== '');
  const title = linesBeforeIngredients[linesBeforeIngredients.length - 1].trim();
...
const parseRecipeStringFr = (text) => {
  ...
  let titleStartIndex = text.indexOf("Voici la recette de la") + "Voici la recette de la".length;
  ...
  const instructionsStart = text.indexOf(instructionsMarker);
  const ingredientsText = text.substring(ingredientsStart, instructionsStart).trim();
  const ingredients = ingredientsText.split('-').filter(line => line.trim() !== '').map(line => line.trim());
...
const parseRecipeStringAr = (text) => {
  ...
  // تحديد العلامات لكل قسم
    ...
    const title = text.substring(0, titleEndIndex).replace("طيب! إليك وصفة ل", "").trim();

    // استخراج المكونات بين "المكونات:" و "الطريقة:"
    const ingredientsStartIndex = text.indexOf(ingredientsMarker) + ingredientsMarker.length;
    const methodStartIndex = text.indexOf(methodMarker);
    const ingredientsText = text.substring(ingredientsStartIndex, methodStartIndex).trim();
    const ingredients = ingredientsText.split('-').filter(line => line.trim() !== '').map(line => line.trim());

    // استخراج الطريقة بعد "الطريقة:"
    const methodText = text.substring(methodStartIndex + methodMarker.length).trim();
    const instructions = methodText.split('\n').filter(line => line.match(/^\d/)).map(line => line.trim());
...
const languageRealtor = {
    ...
    french: {Ingredients: "Ingrédients:",Instructions :"Instructions:"},
